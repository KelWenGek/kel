...
function drawGrid(color, stepx, stepy) {
  ...
  context.fillRect(0, 0, context.canvas.width, context.canvas.heihgt);
...
function drawRing() {
  ...
  context.arc(circle.x, circle.y, circle.radius + RING_INTER_RADIUS, 0, Math.PI * 2, false);
...
function drawTickInnerCircle() {
  ...
  context.beginPath();
  context.strokeStyle = 'rgba(0,0,0,0.1)';
  context.arc(circle.x, circle.y, circle.radius + RING_INTER_RADIUS - TICK_WIDTH, 0, Math.PI * 2, false);
  context.stroke();
  context.restore();
}
...
function drawTicks() {
  var radius = circle.radius + RING_INTER_RADIUS,
    ANGLE_MAX = 2 * Math.PI,
